...
namespace Keylol.Controllers.NormalPoint
{
    public partial class NormalPointController
    {
        ...
        /// <param name="includeRelated">是否包含相关据点、别名、发行时间、App ID，默认 false</param>
            ...
            IdType idType = IdType.Id)
        {
            ...
            if (includeRelated && point.Type == NormalPointType.Game)
            {
                pointDTO.SteamAppId = point.SteamAppId;
                pointDTO.DisplayAliases = point.DisplayAliases;
                pointDTO.ReleaseDate = point.ReleaseDate;
                pointDTO.DeveloperPoints = point.DeveloperPoints.Select(p => new NormalPointDTO(p, true)).ToList();
                pointDTO.PublisherPoints = point.PublisherPoints.Select(p => new NormalPointDTO(p, true)).ToList();
